...
面向桥梁有限元建模场景，基于多模态大模型与RAG技术，构建集图纸解析、结构参数管理、规范知识检索和模型自动生成于一体的智能建模平台。通过 Agent 工作流串联图纸识别、参数校验、规范检索与 SAP2000 建模，实现从竣工图 PDF 到有限元模型的辅助建模，并提供多人协作、任务调度、权限控制及模型版本管理能力。
